...
package com.wang.turbine.utils

import java.sql.{Connection, DriverManager, PreparedStatement, ResultSet}

import com.wang.turbine.config.{ConfigurationManager, Params}
...
object CreateMySqlPool {
  ...
  def apply(): GenericObjectPool[MySqlProxy] = {
    ...
    if (this.genericObjectPool == null) {
      this.synchronized {
        // 获取MySQL配置参数
        val jdbcUrl = ConfigurationManager.config.getString(Params.JDBC_URL)
        val jdbcUser = ConfigurationManager.config.getString(Params.JDBC_USER)
        val jdbcPassword = ConfigurationManager.config.getString(Params.JDBC_PASSWORD)
        val size = ConfigurationManager.config.getInt(Params.JDBC_DATASOURCE_SIZE)
